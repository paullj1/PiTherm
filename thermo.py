...
while True:
	...
	occupied = check_occupancy(db)
	indoor_temp = get_temp(db)
	mode = get_value_from_id(db, MODE_ID)
	fan_on = fan_status(db)
	override = override_status(db)
	setpoint = get_setpoint(db,mode,occupied,override)
